"use client";

import { Quote } from "lucide-react";
import { useLanguage } from "./LanguageProvider";
import testimonialsData from "../data/testimonials.json";
import { useEffect, useState } from "react";

type Testimonial = {
  name: string;
  role: string;
  text: string;
};

export default function Testimonials() {
  const { lang, t } = useLanguage();
  const stories = (testimonialsData[lang as keyof typeof testimonialsData] || []) as Testimonial[];
  
  // Start at the beginning of the second set to allow infinite looping
  const [currentIndex, setCurrentIndex] = useState(stories.length || 0);
  const [isTransitioning, setIsTransitioning] = useState(false);

  // Initialize index securely after hydration
  useEffect(() => {
    if (stories.length > 0) {
      setCurrentIndex(stories.length);
    }
  }, [stories.length]);

  // Handle the 3-second pause and slide
  useEffect(() => {
    if (!stories.length) return;

    const timer = setInterval(() => {
      setIsTransitioning(true);
      setCurrentIndex((prev) => prev + 1);
    }, 3000);

    return () => clearInterval(timer);
  }, [stories.length]);

  // Handle the silent reset for the infinite illusion
  useEffect(() => {
    if (!stories.length) return;

    // When it reaches the exact end of the middle set
    if (currentIndex === stories.length * 2) {
      const timeout = setTimeout(() => {
        setIsTransitioning(false); // Turn off CSS transition
        setCurrentIndex(stories.length); // Instantly snap back to the start of the middle set
      }, 700); // This MUST match the duration-700 in the CSS transition below
      
      return () => clearTimeout(timeout);
    }
  }, [currentIndex, stories.length]);

  if (!stories.length) return null;

  // Render exactly 3 identical sets: [Prev, Current, Next]
  const carouselStories = [...stories, ...stories, ...stories];
  
  // RTL layouts must translate in the positive direction to slide correctly
  const isRTL = lang === "ar";
  const directionMultiplier = isRTL ? 1 : -1;

  return (
    <section className="py-24 bg-gray-50 relative">
      <div className="absolute top-0 w-full h-1/2 bg-white -z-10" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <div className="flex items-center justify-center gap-3 mb-4">
            <div className="w-10 h-1 bg-brand rounded-full" />
            <h2 className="text-brand font-bold uppercase tracking-wide text-2xl md:text-3xl">
              {t("testimonialsTag")}
            </h2>
            <div className="w-10 h-1 bg-brand rounded-full" />
          </div>
          <h3 className="text-3xl md:text-5xl font-extrabold text-gray-900">
            {t("testimonialsTitle")}
          </h3>
        </div>

        <div className="overflow-hidden text-start">
          {/* Transition wrapper */}
          <div 
            className="flex w-max"
            style={{
              transform: `translateX(calc(${currentIndex} * ${directionMultiplier} * min(88vw, 34rem)))`,
              transition: isTransitioning ? "transform 700ms cubic-bezier(0.4, 0, 0.2, 1)" : "none"
            }}
          >
            {carouselStories.map((story, index) => (
              <div key={`story-${index}`} className="w-[min(88vw,34rem)] shrink-0 px-2 sm:px-3">
                <div className="h-full bg-white p-10 lg:p-12 rounded-[2rem] shadow-xl shadow-gray-200/50 border border-gray-100 relative group hover:-translate-y-2 transition duration-500">
                  <div className="absolute top-10 end-10 text-brand/5 group-hover:text-brand/20 transition duration-300">
                    <Quote className={`w-10 h-10 ${lang === "ar" ? "" : "rotate-180"}`} />
                  </div>
                  <div className="flex items-center gap-5 mb-10 relative z-10">
                    <div>
                      <h4 className="font-extrabold text-gray-900 text-xl">{story.name}</h4>
                      <p className="text-brand font-bold text-sm mt-1">{story.role}</p>
                    </div>
                  </div>
                  <p className="text-gray-600 leading-relaxed text-lg relative z-10 font-medium">
                    {story.text}
                  </p>
                </div>
              </div>
            ))}
          </div>
        </div>
      </div>
    </section>
  );
}
